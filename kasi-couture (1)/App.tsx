...
import { supabase, isSupabaseConfigured, fetchCollection, syncToCloud } from './lib/supabase';
...
const SaveStatusIndicator = ({ status }: { status: SaveStatus }) => {
  ...
  return (
    <div className={`fixed bottom-6 right-6 z-[100] flex items-center gap-3 px-4 py-3 rounded-full shadow-2xl transition-all duration-300 ${status === 'error' ? 'bg-red-500 text-white' : 'bg-slate-900 text-white border border-slate-800'} animate-in slide-in-from-bottom-4`}>
      ...
      <span className="text-[10px] font-black uppercase tracking-widest">{status === 'saving' ? 'Syncing...' : status === 'saved' ? 'Cloud Updated' : 'Sync Failed'}</span>
    </div>
  );
};
    ...
    const trackGeo = async () => {
      ...
      try {
        ...
        if (isSupabaseConfigured) {
          supabase.from('analytics_geo').insert([geoEntry]).then(() => {});
        }
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const hydrateData = async () => {
      ...
      try {
        // Fetch Settings
        const { data: cloudSettings, error } = await supabase.from('site_settings').select('config').limit(1).single();
        if (cloudSettings) {
          setSettings(cloudSettings.config);
        } else {
          // First time setup - push INITIAL_SETTINGS to cloud
          await supabase.from('site_settings').insert([{ id: 'global', config: INITIAL_SETTINGS }]);
          setSettings(INITIAL_SETTINGS);
        }

        // Hydrate other collections from local to cloud if necessary
        const hydrated = localStorage.getItem('supabase_hydrated');
        if (!hydrated) {
          const localProducts = JSON.parse(localStorage.getItem('admin_products') || '[]');
          if (localProducts.length > 0) {
            await supabase.from('products').upsert(localProducts);
          }
          localStorage.setItem('supabase_hydrated', 'true');
        }
      } catch (e) {
  ...
  const updateSettings = useCallback(async (newSettings: Partial<SiteSettings>) => {
    ...
    // Always update local for speed
    localStorage.setItem('site_settings', JSON.stringify(updated));

    if (isSupabaseConfigured) {
      const { error } = await supabase.from('site_settings').upsert({ id: 'global', config: updated });
