...
import { LayoutGrid, ShieldCheck, Globe, Sparkles } from 'lucide-react';
    ...
    <main className="pt-0">
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6">
          ...
          <div className="grid grid-cols-4 md:grid-cols-4 gap-4 md:gap-8">
            {categories.slice(0, 4).map((cat) => {
              ...
              return (
                <button key={cat.id} onClick={() => navigate(`/products?category=${cat.id}`)} className="flex flex-col items-center group">
                  <div className="w-12 h-12 md:w-20 md:h-20 bg-slate-50 rounded-[1.5rem] md:rounded-[2rem] flex items-center justify-center text-slate-400 group-hover:bg-primary/10 group-hover:text-primary transition-all duration-500 shadow-sm border border-transparent group-hover:border-primary/20">
                    <Icon size={18} className="md:w-7 md:h-7" strokeWidth={1.5} />
                  </div>
                  <span className="mt-3 md:mt-5 text-[7px] md:text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 group-hover:text-slate-900 truncate w-full text-center">{cat.name}</span>
                </button>
              );
            })}
          </div>
      ...
      <section className="py-12 md:py-32 bg-[#FDFCFB] relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-6 relative z-10">
           <div className="text-center mb-8 md:mb-16"><span className="text-[10px] font-black uppercase tracking-[0.6em] text-primary block mb-2">{settings.homeTrustSectionTitle}</span></div>
           <div className="grid grid-cols-2 md:grid-cols-3 gap-x-4 gap-y-8 md:gap-16">
              {[
                ...
                const Icon = CustomIcons[item.iconName] || (LucideIcons as any)[item.iconName] || ShieldCheck;
                return (
                  <div key={i} className={`flex flex-col items-center text-center ${i === 2 ? 'col-span-2 md:col-span-1' : ''}`}>
                    <div className="mb-4 md:mb-8 p-4 md:p-6 bg-white rounded-3xl shadow-xl border border-slate-50 text-primary hover:scale-110 transition-transform duration-500"><Icon size={20} className="md:w-8 md:h-8" strokeWidth={1.5} /></div>
