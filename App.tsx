...
interface SettingsContextType extends MaisonData {
  ...
  // CRUD actions exposed globally for ease
  updateProducts: (data: Product[]) => void;
  updateCategories: (data: Category[]) => void;
  updateSubCategories: (data: SubCategory[]) => void;
  updateHeroSlides: (data: CarouselSlide[]) => void;
  updateEnquiries: (data: Enquiry[]) => void;
}
...
const App: React.FC = () => {
  ...
  const [maisonData, setMaisonData] = useState<MaisonData>({
    products: INITIAL_PRODUCTS,
    categories: INITIAL_CATEGORIES,
    subCategories: INITIAL_SUBCATEGORIES,
    heroSlides: INITIAL_CAROUSEL,
  ...
  const refreshData = useCallback(async () => {
    ...
    try {
      const [sets, prods, cats, subs, slides, enqs, st] = await Promise.all([
        db.getAll<SiteSettings>('site_settings'),
        db.getAll<Product>('products'),
        db.getAll<Category>('categories'),
        db.getAll<SubCategory>('subcategories'),
        db.getAll<CarouselSlide>('carousel_slides'),
        db.getAll<Enquiry>('enquiries'),
        db.getAll<ProductStats>('product_stats')
      ]);

      // Seed if settings empty
      if (sets.length === 0) {
        console.log("Database empty. Initializing seed...");
        await db.upsert('site_settings', { ...INITIAL_SETTINGS, id: 'main' });
        // Optionally seed other tables if desired
      ...
      setMaisonData({
        ...
        categories: cats.length > 0 ? cats : INITIAL_CATEGORIES,
        subCategories: subs.length > 0 ? subs : INITIAL_SUBCATEGORIES,
        heroSlides: slides.length > 0 ? slides : INITIAL_CAROUSEL,
        enquiries: enqs.length > 0 ? enqs : INITIAL_ENQUIRIES,
        stats: st
      });
    } catch (err) {
  ...
  const updateSettings = async (newSettings: Partial<SiteSettings>) => {
    ...
    try {
      const updated = { ...settings, ...newSettings, id: 'main' };
      if (isSupabaseConfigured) {
        await db.upsert('site_settings', updated);
      }
      setSettings(updated);
      setSaveStatus('saved');
    } catch (e) {
  ...
  return (
    <SettingsContext.Provider value={{ 
      ...
      setSaveStatus,
      logEvent,
      updateProducts: (data) => setMaisonData(p => ({...p, products: data})),
      updateCategories: (data) => setMaisonData(p => ({...p, categories: data})),
      updateSubCategories: (data) => setMaisonData(p => ({...p, subCategories: data})),
      updateHeroSlides: (data) => setMaisonData(p => ({...p, heroSlides: data})),
      updateEnquiries: (data) => setMaisonData(p => ({...p, enquiries: data}))
    }}>
      <Router>
        <ScrollToTop />
